...
const ModuleList = () => {
  ...
  const handleyesno = async (id) => {
    try {
      const response = await fetch(`http://localhost:5000/api/package/deleteitem/${id}`, {
        method: "DELETE",
      });
...
//for edit
  const handleedit = async (id) => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/api/package/getitemByid/${id}`, {
        method: "GET",
      });
      const data = await response.json();
  ...
  const handleChange = async (status,id) => {
    ...
    try {
      ...
      const response = await fetch("http://localhost:5000/api/package/update-statusitem", {
        method: "PATCH",
        headers: {
      ...
      if (response.ok == true) {
        fetchData();
        toast.success('Fixed Item Status updated Successfully');
  ...
  const fetchData = async () => {
      ...
      try {
        ...
        const response = await fetch(`http://localhost:5000/api/package/getdata`,{
          method:"GET",
      });
  ...
  const handleaddsubmit = async (e) => {
    ...
    if (Object.keys(newErrors).length > 0) {
      ...
      if (!id) {
        try {
          const response = await fetch(`http://localhost:5000/api/package/additem`, {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
            },
          ...
          if (response.ok == true) {
            handleClose1();
            toast.success('Fixed Item Added successfully!');
            setErrors('');
            setfeatures({
              name: "",
              details: "",
        ...
        try {
          const response = await fetch(`http://localhost:5000/api/package/updateItem/${id}`, {
            method: "PATCH",
            headers: {
              "Content-Type": "application/json",
            },
  ...
  return (
    <Container className="list-module-preview">
      ...
      <Modal show={show1} onHide={handleClose1} className="other-icon-modal">
        ...
        <Modal.Body>
          <h3>{!itemIdToDelete ? 'Add' : 'Edit'} Fixed Item Master</h3>
